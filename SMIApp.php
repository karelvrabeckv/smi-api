...
/**
 * File for Application Form Class.
 */

/**
 * Submit Class for application form.
 *
 * This class includes all application/registration related functions used for input/output of student data.
 *
 * @author 	Reuben Debattista (rdebattista@stmartins.edu)
 * @date	July 2019
 *
 * @link	https://api.stmartins.edu/rest/site/applicationForm Application Form CLASS URI
 * @package REST\Site\AppForm
 * @version Stable-8.0
 *
 */
...
class SMIApp extends REST {
	...
	public function updateMacAddress($params){
		...
		$locID = $this->sanitize($this->_request['locationid'], 'int');
		$macAddr = $this->sanitize($this->_request['macAddress'], 'string');
		$accessToken = $this->sanitize($this->_request['access_token'], 'string');
		...
		if ($auth) {
			...
			$value = 1;
			//$value = $this->sanitize($this->_request['msgid'], 'int');
			$stmtDetails = $connect->prepare('UPDATE locations
											  SET macAddress = ?
											  WHERE locationid = ?');
							$stmtDetails->bind_param('si', $macAddr, $locID);
							$stmtDetails->execute();							
							$stmtDetails->close();						
	...
	public function saveAutoAttendance($params){
		...
		if ($auth) {
			...
			if ($countRows > 0) {
				...
				if (($attendanceState == 0) || ($attendanceState == 1)) {
					...
					// Write to Table
					$stmt_msg = $connect->prepare('INSERT INTO ttableattendance(ttableentryid, userid, automaticAttendance) VALUES(?,?,?);');
	...
	public function getAutoAttendanceDev($params){
		...
		if ($auth) {
			...
			// Does an entry already exist?
			$stmt_msg = $connect->prepare('SELECT ttableattendance.userid, ttableattendance.automaticAttendance 
											FROM ttableattendance 
											Where ttableentryid = ?
										   ');
	...
	public function getAutoAttendance($params){
	...
	public function getLocations($params){
		...
		if ($auth) {		
			...
			$value = 1;
			//$value = $this->sanitize($this->_request['msgid'], 'int');
			$stmtDetails = $connect->prepare('SELECT * FROM locations;');						
							$stmtDetails->execute();
							$result = $stmtDetails->get_result();
							$stmtDetails->close();						
	...
	public function getCurrentClassMacAddressAndRssi() {
		...
		// Parameters from HTTP query		
		$userid = $this->sanitize($this->_request['userid'], 'int');
		$time = $this->sanitize($this->_request['time'], 'string');
		$date = $this->sanitize($this->_request['date'], 'string');
		$accessToken = $this->sanitize($this->_request['access_token'], 'string');
		...
		if ($auth) {
			...
			# Read from table		
			$stmtDetails = $connect->prepare('SELECT tta.userid, u.username , u.name, u.surname , u.email, tt.start, tt.end, c.name, c.description, tte.ttableentryid, tt.ttableid, tte.ttabledate, l.location AS roomNumber, l.locationid, l.macAddress, l.rssiPower 
											FROM Users u
											JOIN ttableattendance tta ON u.userid = tta.userid              
											JOIN ttableentries tte ON tta.ttableentryid = tte.ttableentryid 
											JOIN ttable tt ON tt.ttableid = tte.ttableid                    
											...
											WHERE tta.userid = ? AND Datediff(?, tte.ttabledate) = 0 AND ? BETWEEN cast(tt.start as time(0)) AND cast(tt.end as time(0)) -- 3727 , 2020-03-05 and 09:00:00 should be parameters in the api
											Order by HOUR(tt.start)');	
							$stmtDetails->bind_param('iss',$userid, $date, $time);											
							$stmtDetails->execute();
							$result = $stmtDetails->get_result();
							$stmtDetails->close();						
	...
	public function updateRssi($params){
		...
		$locID = $this->sanitize($this->_request['locationid'], 'int');
		$rssi = $this->sanitize($this->_request['rssiPower'], 'string');
		$accessToken = $this->sanitize($this->_request['access_token'], 'string');
		...
		if ($auth) {
			...
			$stmtDetails = $connect->prepare('UPDATE locations
											  SET rssiPower = ?
											  WHERE locationid = ?');
							$stmtDetails->bind_param('si', $rssi, $locID);
							$stmtDetails->execute();							
							$stmtDetails->close();						
